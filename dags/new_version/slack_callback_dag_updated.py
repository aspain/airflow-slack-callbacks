...
from include.new_version import slack_callback_functions_updated
...
default_args = {
    ...
    "on_success_callback": slack_callback_functions_updated.success_callback,
    "on_failure_callback": slack_callback_functions_updated.failure_callback,
    "on_retry_callback": slack_callback_functions_updated.retry_callback,
    "sla": timedelta(seconds=10),
}
with DAG(
    dag_id="slack_callbacks_updated",
    ...
    sla_miss_callback=slack_callback_functions_updated.sla_miss_callback,
    doc_md=__doc__,
    catchup=False,
) as dag:
    ...
    empty_trigger = EmptyOperator(
        ...
        on_execute_callback=slack_callback_functions_updated.dag_triggered_callback,
        on_success_callback=None,
    )

    # This task uses the default_args on_success_callback
    empty_success_test = EmptyOperator(
        task_id="empty_success_test",
    )

    # This task sends a Slack message via a python_callable
    slack_test_func = PythonOperator(
        task_id="slack_test_func",
        python_callable=slack_callback_functions_updated.slack_test,
        on_success_callback=None,
    )
    ...
    empty_dag_success = EmptyOperator(
        ...
        on_success_callback=slack_callback_functions_updated.dag_success_callback,
